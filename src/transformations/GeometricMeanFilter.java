 ...
 * @date 05/20/2018
 */
public class GeometricMeanFilter implements TransformationAlgorithm {
	
	/**
	 * Creates an object for geometric mean filter
	 * @param m the Model for MVC.
	 */
	public GeometricMeanFilter(Model m) {
	...
	public int[][] transform(int[][] image) {
		...
		for (int yN = 0; yN < size; yN ++) {
			for (int xN = 0; xN < size; xN ++) {
				...
				// multiply the gray values in the mask
				double mult = 1;
				...
				for (int yO = yN - maskOffset; yO <= yN + maskOffset; yO ++) {
					for (int xO = xN - maskOffset; xO <= xN + maskOffset; xO ++) {
						if (!(yO < 0 || yO >= size || xO < 0 || xO >= size)) {
							count ++;
							mult *= image[yO][xO] & 0xff;
						}
					}
				}
				
				// get the new gv as the average of the mask
				int gv = (int) Math.pow(mult, 1.0/count);
